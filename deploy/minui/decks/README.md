...
on-device import step needed.

```sh
cp ~/Downloads/Spanish101.apkg deploy/minui/decks/
make package
```

Multiple files accumulate into one database (imports are additive). To
use specific files without touching this folder, pass `--deck`
explicitly instead (this folder is then ignored):
