#!/usr/bin/env bash
# Assemble the deployable CardBrick package for MinUI / TrimUI Brick.
#
#   deploy/minui/dist/
#   ├── CardBrick.sh                 # launch script (ash-compatible)
#   ├── CardBrick/                   # goes alongside CardBrick.sh
...
#   └── CardBrick-minui-v<ver>.zip   # same content, zipped
#
# Usage:
#   scripts/build_package.sh                 # runtime from runtime-build/
...
MINUI_DIR="$(cd "${HERE}/.." && pwd)"
REPO_ROOT="$(cd "${MINUI_DIR}/../.." && pwd)"
APP_SRC="${REPO_ROOT}/cardbrick-py"
DIST="${MINUI_DIR}/dist"
STAGE="${DIST}/CardBrick"
DECKS_DIR="${MINUI_DIR}/decks"
...
# ----------------------------------------------------------- find runtime
# Unlike deploy/knulli, the MinUI runtime-build's squashfs lands directly
# in runtime-build/ (no build/ subdir) — see runtime-build/docker-compose.yml.
RUNTIME_FILE=""
if [ "$WITH_RUNTIME" -eq 1 ]; then
    if [ -n "${RUNTIME_SQUASHFS:-}" ]; then
        RUNTIME_FILE="$RUNTIME_SQUASHFS"
    else
        RUNTIME_FILE="$(ls -1t "${MINUI_DIR}/runtime-build/"pygame-ce_*.squashfs 2>/dev/null | head -n 1 || true)"
    fi
...
# Copy an ALLOWLIST of what actually ships, rather than the whole
# cardbrick-py/ directory minus excludes. A denylist misses local dev
# cruft that doesn't have a fixed name or location — most commonly a
# virtualenv created inside cardbrick-py/ (.venv, venv, env), whose
# installed packages routinely bundle their own nested tests/ and
# __pycache__ dirs. Copying only the known app paths makes that class
...
install -m 755 "${MINUI_DIR}/CardBrick.sh" "${DIST}/CardBrick.sh"
...
# no on-device import step. Imports are additive, so multiple files
# just accumulate into one database.
...
ZIP_NAME="CardBrick-minui-v${VERSION}${SUFFIX}.zip"
