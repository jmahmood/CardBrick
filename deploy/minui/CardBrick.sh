#!/bin/sh
# CardBrick — MinUI / TrimUI Brick (tg5040) launch script (pygame-ce
# squashfs runtime).
#
# Lives alongside the CardBrick/ folder on the device (see
# deploy/minui/scripts/build_package.sh). MinUI runs pak launch
# scripts under ash (BusyBox), not bash — this script is POSIX sh,
# with no bashisms (arrays, [[ ]], PIPESTATUS, etc.).
...
# and the app runs on that interpreter.
#
# Usage (also over SSH):
...
# ------------------------------------------------------------------ SDL
# `mali` is what was validated on-device for this MinUI/pygame-ce
# runtime (TrimUI Brick). If the screen stays black try kmsdrm:
...
if [ "$#" -gt 0 ]; then
    # Passthrough mode (e.g. --smoke-test over SSH). ash has no
    # PIPESTATUS/tee-and-preserve-exit-code trick like bash, so just
    # redirect straight into the log instead of teeing to the terminal.
    "$PYTHON" main.py --knulli "$@" >> "$LAUNCH_LOG" 2>&1
    exit $?
fi
