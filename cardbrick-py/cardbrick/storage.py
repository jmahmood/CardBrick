...
# Valid reasons a learner can pick from the keyboard-free flag menu. Kept as a
# closed set so the enum is the whole signal (no free text on the device) and
# the offline pipeline can branch on it deterministically.
FLAG_REASONS = (
    "wrong_translation",   # EN/JP translation doesn't carry the word's meaning
    "meaning_not_listed",  # sentence uses a sense (often colloquial) not taught
    "inappropriate",       # not suitable for a young learner
    "other",
)
...
-- Learner-raised reports that an example sentence is wrong or inappropriate.
-- Raised from a keyboard-free multiple-choice picker during review, so the
-- ``reason`` enum is the whole signal (``note`` is reserved for the offline
-- pipeline / a parent, never typed on the device). ``example_snapshot`` keeps
-- the sentence as it was when flagged, so a later content re-import cannot
-- erase the evidence a flag was raised about.
...
class Storage:
    ...
    # Columns the flag-resolution pipeline is allowed to rewrite on a vocab
    # card. Excludes card_id (the key) and audio (regenerated by import, not by
    # a text pipeline).
    _VOCAB_EDITABLE = frozenset((
        "word", "word_en", "word_jp", "gendered_forms", "definitions",
        "image_filename", "example_es", "example_en", "example_jp",
        "report_link",
    ))

    def update_vocab_fields(self, card_id, fields):
        """Partial content update for a vocab card (used by ``flags apply``).

        Only whitelisted columns are touched; review_state is never involved,
        so FSRS progress is preserved exactly like a re-import. Returns the
        number of rows changed (0 if the card_id has no vocab row)."""
        clean = {k: v for k, v in fields.items() if k in self._VOCAB_EDITABLE}
        if not clean:
            return 0
        assignments = ", ".join(f"{col}=:{col}" for col in clean)
        clean["card_id"] = card_id
        cursor = self.conn.execute(
            f"UPDATE vocab_cards SET {assignments} WHERE card_id=:card_id",
            clean)
        self.conn.commit()
    ...
    def add_sentence_flag(self, card_id, reason, snapshot=None, now_iso=None):
        """Record a learner report about a card's example sentence.

        ``reason`` is one of ``FLAG_REASONS``; ``snapshot`` is an optional JSON
        blob of the example fields as they were when flagged. No ``note`` here:
        nothing on the handheld can type one.
        """
        if reason not in FLAG_REASONS:
            raise ValueError("unknown flag reason: %r" % (reason,))
        cursor = self.conn.execute(
            """INSERT INTO sentence_flags
                   (card_id, reason, example_snapshot, status, created_at)
               VALUES (?, ?, ?, 'open', ?)""",
            (card_id, reason, snapshot, now_iso))
        self.conn.commit()
        return cursor.lastrowid

    def open_flags(self):
        """Open flags joined to their vocab content, newest first.

        The join supplies everything the offline pipeline needs to judge a
        sentence — the word, its listed meanings, and every translation — so
        ``flags export`` is a single query.
        """
        return self.conn.execute(
            """SELECT f.id AS flag_id, f.card_id, f.reason, f.note,
                      f.example_snapshot, f.status, f.created_at,
                      c.deck, c.front,
                      v.word, v.word_en, v.word_jp, v.definitions,
                      v.example_es, v.example_en, v.example_jp,
                      v.image_filename
               FROM sentence_flags f
               JOIN cards c ON c.id = f.card_id
               LEFT JOIN vocab_cards v ON v.card_id = f.card_id
               WHERE f.status = 'open'
               ORDER BY f.created_at DESC, f.id DESC"""
        ).fetchall()

    def flags_for_card(self, card_id):
        return self.conn.execute(
            "SELECT * FROM sentence_flags WHERE card_id = ? ORDER BY id",
            (card_id,)).fetchall()

    def resolve_flag(self, flag_id, resolution, now_iso=None):
        """Mark a flag done, recording what the pipeline did about it."""
        self.conn.execute(
            """UPDATE sentence_flags
                   SET status='resolved', resolution=?, resolved_at=?
                   WHERE id=?""",
            (resolution, now_iso, flag_id))
        self.conn.commit()

    def dismiss_flag(self, flag_id, now_iso=None):
        """Close a flag without any content change (false alarm)."""
        self.conn.execute(
            """UPDATE sentence_flags
                   SET status='dismissed', resolution='no_change',
                       resolved_at=?
                   WHERE id=?""",
            (now_iso, flag_id))
        self.conn.commit()
