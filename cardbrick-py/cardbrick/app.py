...
class CardBrickApp:
    ...
    def screen_child_start(self):
        ...
        while True:
            ...
            self.screen.fill(BG)
            self._center(self.font_small.render("SPANISH PRACTICE", True, DIM), 36)
            self._center(self.font_big.render(self.profile["name"], True, FG), 80)
            ...
            self._center(
                self.font_small.render(
                    self._truncate_to_width(self.font_small, deck_label, self.w - 40),
                    True,
                    DIM,
                ),
                182,
            ...
            if startable:
                ...
                if status["cards_done"] == 0:
                    headline = (
                        "Just one sprint today!" if n == 1 else f"{n} sprints today"
                    )
                else:
                    headline = (
                        "Last sprint of the day!"
                        if n == 1
                        else f"{n} sprints to go today"
                    )
                ...
                self._center(self.font_big.render(headline, True, GOOD), 215)
                self._center(
                    self.font.render(
                ...
                self._center(
                    self.font_big.render("All done for today!", True, GOOD), 230
                )
    ...
    def screen_deck_select(self):
        ...
        while True:
            ...
            self.screen.fill(BG)
            self._center(self.font_big.render("Choose a Deck", True, FG), 40)
            self._center(
                self.font_small.render("Which deck do you want to study?", True, DIM),
                90,
            )
            y = 140
            for i in range(top, min(top + visible, len(entries))):
                ...
                text = self._truncate_to_width(
                    self.font, f"{prefix}{label}   ({due + new} due)", self.w - 120
                )
    ...
    def screen_category_select(self):
        ...
        while True:
            ...
            self._center(
                self.font_small.render(
                    "Want to focus on something specific?", True, DIM
                ),
                90,
            )
            y = 140
    ...
    def _draw_review(
        ...
        header = self._truncate_to_width(self.font_small, header, max_left_width)
        self.screen.blit(self.font_small.render(header, True, DIM), (16, 12))
        self.screen.blit(right_surf, (self.w - right_surf.get_width() - 16, 12))
        ...
        y = self._block(front, self.font_big, FG, top=64, max_width=max_width)

        if audio_status == "missing":
            self._center(self.font_small.render("(no audio)", True, WARN), y + 6)
        elif card["audio_filename"]:
            self._center(self.font_small.render("♪  L1 = replay", True, DIM), y + 6)

        if flipped:
            div_y = max(y + 34, 210)
            pygame.draw.line(
                self.screen, DIVIDER, (margin, div_y), (self.w - margin, div_y)
            )
    ...
    def _draw_vocab_phases(self, card, vocab, phase, audio_status, top):
        ...
        margin = 36
        max_width = self.w - 2 * margin
        y = self._block(vocab["word"], self.font_big, FG, top=top, max_width=max_width)
        ...
        if phase >= 2:
            ...
            if image is not None:
                ...
                self._center(self.font_small.render("(no image)", True, DIM), y)
                y += 26
    ...
    def _draw_highlighted_block(self, text, word, font, top, max_width):
        ...
        for line in wrap_text(font, text, max_width):
            ...
            if idx == -1:
                ...
                self._blit_split_highlight(line, idx, len(word_lower), font, y)
            y += font.get_linesize()
        return y
    ...
    def _blit_split_highlight(self, line, start, length, font, y):
        ...
        total_w = surf_pre.get_width() + surf_match.get_width() + surf_post.get_width()
        x = (self.w - total_w) // 2
    ...
    def _draw_menu_overlay(self, index):
        ...
        pygame.draw.rect(self.screen, DIVIDER, (x, y, box_w, box_h), 2, border_radius=8)
    ...
    def screen_summary(self):
        ...
        lines = [
            ...
            (
                ...
                f"Reviews:  {s.get('review_cards', 0)}",
                FG,
            ),
            (
            ...
            (
                ...
                + (f"    Avg answer:  {avg / 1000:.1f}s" if avg else ""),
                DIM,
            ),
            (f"Today:  {status['cards_done']} / {status['goal_today']} cards", DIM),
        ]
    ...
    def _draw_day_cell(self, day, count, is_today, x, y, w, h):
        if is_today:
            pygame.draw.rect(
                self.screen, ACCENT, (x + 2, y, w - 4, h - 2), 2, border_radius=6
            )
    ...
    def screen_parent_menu(self):
        ...
        while True:
            ...
            self._center(
                self.font_small.render(f"Profile: {self.profile['name']}", True, DIM),
                90,
            )
            y = 124
            for i in range(top, min(top + visible, len(entries))):
                ...
                color = ACCENT if i == index else FG
                prefix = "> " if i == index else "   "
                self.screen.blit(self.font.render(prefix + label, True, color), (90, y))
                y += 40
            if notice:
    ...
    def _draw_import(self, files, index, message):
        ...
        if not files:
            ...
            for i, path in enumerate(files[:7]):
                ...
                prefix = "> " if i == index else "   "
                self.screen.blit(
                    self.font.render(prefix + os.path.basename(path), True, color),
                    (70, y),
                )
                y += 38
        if message:
    ...
    def _screen_multi_select(
        ...
        while True:
            ...
            for i in range(top, min(top + visible, len(entries))):
                ...
                self.screen.blit(self.font.render(text, True, color), (80, y))
                y += 40
            if not items:
    ...
    def screen_parent_limits(self):
        ...
        while True:
            ...
            self.screen.fill(BG)
            self._center(self.font_big.render("Daily Goal & Sprints", True, FG), 40)
            sprint_size = max(values["session_card_limit"], 1)
            sprints = -(-values["daily_goal_cards"] // sprint_size)
            self._center(
                self.font_small.render(
                    f"= {sprints} sprint{'s' if sprints != 1 else ''} a day", True, DIM
                ),
                92,
            )
            y = 140
            for i, (label, key, _, _) in enumerate(fields):
                color = ACCENT if i == index else FG
                prefix = "> " if i == index else "   "
                self.screen.blit(self.font.render(prefix + label, True, color), (70, y))
    ...
    def screen_parent_suspended(self):
        ...
        while True:
            ...
            self.screen.fill(BG)
            self._center(self.font_big.render("Suspended Cards", True, FG), 36)
            if not cards:
    ...
    def _footer(self, text, second_line=None):
        ...
        pygame.draw.line(self.screen, DIVIDER, (16, top), (self.w - 16, top))
