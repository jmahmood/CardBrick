...
mod sprint2_tests {
    ...
    fn create_test_bandit(param_id: &'static str, defaults: &[f32], db_path: &std::path::Path) -> Result<Bandit, Box<dyn std::error::Error>> {
        Bandit::load_with_path(param_id, defaults, db_path)
    }

    /// Save bandit state to custom database path for testing - NO GLOBAL STATE
    fn save_test_bandit(bandit: &mut Bandit, _param_id: &str, db_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn test_a1_sample_highest_mean_arm_often() {
        ...
        let mut bandit = create_test_bandit("test_highest_mean", &[10.0, 20.0, 30.0], &db_path).unwrap();
        
        // Manually set different priors to create clear expectations
        bandit.arms[0].alpha = 3; bandit.arms[0].beta = 1;  // mean ≈ 0.75
        bandit.arms[1].alpha = 1; bandit.arms[1].beta = 1;  // mean = 0.5  
        bandit.arms[2].alpha = 1; bandit.arms[2].beta = 2;  // mean ≈ 0.33
        ...
        assert!(arm_a_count > arm_b_count, "Arm A should be selected more than Arm B");
        assert!(arm_a_count > arm_c_count, "Arm A should be selected more than Arm C");
        assert!(arm_a_count as f64 / trials as f64 > 0.4, "Arm A should be selected >40% of time");
    }
    ...
    fn test_a2_sampling_updates_beta_priors_correctly() {
        ...
        println!("Before update: Alpha={}, Beta={}", bandit.arms[0].alpha, bandit.arms[0].beta);
        ...
        println!("After reward=1: Alpha={}, Beta={}", bandit.arms[0].alpha, bandit.arms[0].beta);
        ...
        println!("After reward=0: Alpha={}, Beta={}", bandit.arms[0].alpha, bandit.arms[0].beta);
    ...
    fn test_a3_respects_cold_start_priors() {
        ...
        let mut bandit = create_test_bandit("test_cold_start", &[5.0, 10.0, 15.0], &db_path).unwrap();
        
        // All arms should initialize with Beta(3,1) priors
        for (i, arm) in bandit.arms.iter().enumerate() {
            println!("Arm {}: Value={}, Alpha={}, Beta={}", i, arm.value, arm.alpha, arm.beta);
    ...
    fn test_b1_selected_parameters_written_to_daily_log() {
        ...
        let mut stmt = conn.prepare(
            "SELECT pack_sz, rev_coef, fail_k FROM daily_log WHERE date = ?1"
        ).unwrap();
        
        let date_str = today.format("%Y-%m-%d").to_string();
        let row = stmt.query_row([&date_str], |row| {
            Ok((
        ...
        println!("Daily log entry: pack_sz={:?}, rev_coef={:?}, fail_k={:?}", row.0, row.1, row.2);
        ...
        assert!(pack_sz >= 9 && pack_sz <= 15, "pack_sz should be in range [9, 15]");
        assert!(rev_coef >= 1.5 && rev_coef <= 2.5, "rev_coef should be in range [1.5, 2.5]");
        assert!(fail_k >= 3 && fail_k <= 7, "fail_k should be in range [3, 7]");
    }
    ...
    fn test_b2_daily_parameters_used_in_study_queue() {
        ...
        for i in 1..=20 {
            conn.execute("INSERT INTO srs_log (card_id, next_due_ts) VALUES (?1, ?2)", 
                        [i, past_timestamp]).unwrap();
        ...
        println!("Sampled parameters: pack_sz={}, rev_coef={}, fail_k={}", pack_sz, rev_coef, fail_k);
        ...
        println!("Queue data: pack_size={}, rev_coef={}, fail_k={}, cards.len()={}",
                queue_data.pack_size, queue_data.review_coefficient, queue_data.fail_k, queue_data.cards.len());
        
        // Verify parameters are used correctly
        assert_eq!(queue_data.pack_size, pack_sz, "Queue pack_size should match sampled value");
        assert_eq!(queue_data.review_coefficient, rev_coef, "Queue rev_coef should match sampled value");
        assert_eq!(queue_data.fail_k, fail_k, "Queue fail_k should match sampled value");
        ...
        assert!(total_cards <= expected_review_cap + expected_new, "Total cards should not exceed caps");
    }
    ...
    fn test_c1_skill_point_reward_capped_and_scaled() {
        ...
        for (points, expected_bin, expected_cont) in test_cases {
            ...
            println!("Points: {}, Binary: {}, Continuous: {:.1}", points, reward_bin, reward_cont);
            
            assert_eq!(reward_bin, expected_bin, "Binary reward incorrect for {} points", points);
            assert_eq!(reward_cont, expected_cont, "Continuous reward incorrect for {} points", points);
        }
    }
    ...
    fn test_c2_reward_efficiency_calculated_correctly() {
        ...
        for (cards_studied, points_gained, expected_efficiency) in test_cases {
            ...
            println!("Cards: {}, Points: {}, Efficiency: {:.1}", 
                    cards_studied, points_gained, efficiency);
            
            assert_eq!(efficiency, expected_efficiency, 
                      "Efficiency incorrect for {} cards, {} points", cards_studied, points_gained);
        }
    }
    ...
    fn test_c3_bandit_arms_updated_from_logged_reward() {
        ...
        println!("Initial state: Alpha={}, Beta={}", initial_alpha, initial_beta);
        ...
        println!("After reward=1: Alpha={}, Beta={}", after_reward_alpha, after_reward_beta);
        
        // Check correct update
        assert_eq!(after_reward_alpha, initial_alpha + 1, "Alpha should increment by 1 for reward=1");
        assert_eq!(after_reward_beta, initial_beta, "Beta should be unchanged for reward=1");
        ...
        println!("After reward=0: Alpha={}, Beta={}", after_penalty_alpha, after_penalty_beta);
        
        // Check correct update
        assert_eq!(after_penalty_alpha, after_reward_alpha, "Alpha should be unchanged for reward=0");
        assert_eq!(after_penalty_beta, after_reward_beta + 1, "Beta should increment by 1 for reward=0");
    ...
    fn test_d1_bandit_sampling_independent_per_parameter() {
        ...
        let mut pack_bandit = create_test_bandit("pack_size", &[9.0, 12.0, 15.0], &db_path).unwrap();
        ...
        assert_eq!(rev_bandit.arms[0].alpha, initial_rev_alpha, "Review bandit should be unaffected");
        assert_eq!(fail_bandit.arms[0].alpha, initial_fail_alpha, "Fail bandit should be unaffected");
    ...
    fn test_d2_multiple_daily_runs_idempotent() {
        ...
        println!("First run: pack_sz={}, rev_coef={}, fail_k={}", 
                queue1.pack_size, queue1.review_coefficient, queue1.fail_k);
        println!("Second run: pack_sz={}, rev_coef={}, fail_k={}", 
                queue2.pack_size, queue2.review_coefficient, queue2.fail_k);
        
        // Parameters should be consistent (UPSERT preserves existing values)
        assert_eq!(queue1.pack_size, queue2.pack_size, "Pack size should be consistent");
        assert_eq!(queue1.review_coefficient, queue2.review_coefficient, "Rev coef should be consistent");
        assert_eq!(queue1.fail_k, queue2.fail_k, "Fail K should be consistent");
        
        // Check daily_log has only one entry
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM daily_log WHERE date = ?1").unwrap();
        let date_str = today.format("%Y-%m-%d").to_string();
    ...
    fn test_e1_invalid_arm_value_fails_gracefully() {
        ...
        conn.execute(
            ...
            ["test_invalid", &99999.99.to_string(), &1.to_string(), &1.to_string()]
        ).unwrap();
    ...
    fn test_e2_reward_update_skipped_for_missing_log() {
        ...
        assert!(bandit_result.is_ok(), "System should recover after handling missing log");
    }
    ...
    fn test_e3_bandit_arm_decay_mechanism() {
        ...
        println!("Before update: Alpha={}, Beta={}, Sum={}", initial_alpha, initial_beta, initial_sum);
        ...
        println!("After update: Alpha={}, Beta={}, Sum={}", final_alpha, final_beta, final_sum);
    ...
    fn test_e4_posterior_saturation_edge_mean_unchanged() {
        ...
        println!("Before decay+update: Alpha={}, Beta={}, Mean={:.6}", 
        ...
        println!("After decay+update: Alpha={}, Beta={}, Mean={:.6}", 
    ...
    fn test_e5_first_six_day_round_robin_exact_cycle() {
        ...
        let bandit = Bandit::load_with_path("test_round_robin_cycle", &[10.0, 20.0, 30.0], &db_path).unwrap();
        ...
        assert_eq!(arm_counts[0], 2, "Arm 10.0 should be selected exactly 2 times");
        assert_eq!(arm_counts[1], 2, "Arm 20.0 should be selected exactly 2 times");
        assert_eq!(arm_counts[2], 2, "Arm 30.0 should be selected exactly 2 times");
        ...
        for (i, (&selected, &expected)) in selected_arms.iter().zip(expected_pattern.iter()).enumerate() {
            assert!((selected - expected).abs() < f32::EPSILON,
                   "Day {} should select arm {}, but selected {}", i, expected, selected);
        }
        ...
        println!("Day 7 (Thompson sampling): Selected arm with value {}", day7_sample);
        ...
        assert!(
            ...
            "Day 7 should select a valid arm value, got {}", day7_sample
        );
    }
    ...
    fn test_comprehensive_integration() {
        ...
        println!("1. Sampled Parameters: pack_sz={}, rev_coef={:.1}, fail_k={}", pack_sz, rev_coef, fail_k);
        ...
        println!("2. Queue Created: {} cards, pack_size={}", queue_data.cards.len(), queue_data.pack_size);
        ...
        println!("3. Simulated Reward: {} points → bin={}, cont={:.1}", simulated_points, reward_bin, reward_cont);
        
        // 5. Apply reward
        let reward_result = super::super::bandit::apply_reward_with_path(today, reward_bin == 1, &db_path);
        ...
        println!("5. Database State: {} bandit entries, {} daily log entries", bandit_count, log_count);
