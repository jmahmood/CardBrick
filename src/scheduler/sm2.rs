...
pub trait Scheduler {
    ...
    fn get_card_note_from_db(
        &mut self,
        card_id: i64,
    ) -> Result<Option<(i64, Note)>, Box<dyn std::error::Error>>;
}
...
impl Scheduler for Sm2Scheduler {
    fn new(deck: Deck) -> Self {
        ...
        let mut review_queue = if let Ok(()) = queue::ensure_today_with_deck(today, &deck) {
            ...
            println!(
                "📅 Loaded deck-specific daily queue with {} remaining cards for {}",
                remaining_cards.len(),
                today
            );
            ...
            let mut limited_cards: Vec<i64> = deck
                .cards
                .iter()
        ...
        println!(
            "🎯 Session will have {} total cards for deck {}",
            session_total, deck_id
        );
    ...
    fn answer_card(&mut self, card_id: i64, rating: Rating) -> Option<Card> {
        ...
        println!(
            "DEBUG: Total session ratings now: {}",
            self.session_ratings.len()
        );
        ...
        match rating {
            ...
            _ => {
                ...
                if !self.today_completed_cards.contains(&card_id) {
                    ...
                    let today = Utc::now().date_naive();
                    if let Err(e) =
                        queue::mark_card_completed_for_deck_id(today, card_id, &self.deck_id)
                    {
                        eprintln!(
                            "Warning: Failed to mark card {} as completed in deck {}: {}",
                            card_id, self.deck_id, e
                        );
                    }
                }
                ...
                match rating {
                    ...
                    Rating::Hard => {
                        ...
                        result_card.ease_factor =
                            (result_card.ease_factor as i32 - 150).max(1300) as u32;
                    ...
                    Rating::Easy => {
                        ...
                        let new_interval = if result_card.interval == 0 {
                            ...
                            (result_card.interval as f32 * ease_factor_multiplier * easy_bonus)
                                .round() as u32
                        };
                        result_card.interval = new_interval.max(result_card.interval + 1);
                    }
    ...
    fn rewind_last_answer(&mut self) -> Option<Card> {
        if let Some((card_id, rating, original_card_state)) = self.last_answer.take() {
            ...
            if let Some(last_pos) = self
                .session_ratings
                .iter()
                .rposition(|&(id, _)| id == card_id)
            {
                self.session_ratings.remove(last_pos);
                println!(
                    "DEBUG: Removed rating from session, now {} ratings",
                    self.session_ratings.len()
                );
            }
...
mod tests {
    ...
    fn create_test_deck(num_cards: usize) -> Deck {
        ...
        for i in 0..num_cards {
            ...
            notes.insert(
                ...
                Note {
                    id: note_id,
                    fields: vec![format!("Front {}", i), format!("Back {}", i)],
                },
            );
        }
        Deck {
    ...
    fn test_continue_studying_prioritizes_difficult_cards() {
        ...
        assert!(
            !scheduler.failed_cards_today().is_empty(),
            "Failed cards should be tracked"
        );
        assert!(
            !scheduler.hard_cards().is_empty(),
            "Hard cards should be tracked"
        );
        ...
        match load_result {
            Ok(new_cards) => {
                assert_eq!(
                    new_cards.len(),
                    crate::config::PACK_SIZE_DEFAULT,
                ...
                println!(
                    "Cards in load_more_cards result: {:?}",
                    new_cards.iter().map(|c| c.id).collect::<Vec<_>>()
                );
                ...
                assert!(
                    loaded_45 || loaded_47,
                    "At least one difficult card should be prioritized from database"
                );
                ...
                assert!(
                    scheduler.review_queue.len() >= crate::config::PACK_SIZE_DEFAULT,
                    "Cards should be added to review queue"
                );
            }
            Err(e) => {
                panic!(
                    "load_more_cards should succeed when database has available cards: {}",
                    e
                );
            }
        }
