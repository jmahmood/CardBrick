...
mod tests {
    ...
    fn test_30_day_study_simulation() {
        ...
        while current_date <= end_date {
            ...
            match session_result {
                Ok((points_earned, cards_studied, goal_achieved)) => {
                    ...
                    println!("📅 Day {}: {} points, {} cards, goal: {}", 
                            day_number, points_earned, cards_studied, 
        ...
        let avg_daily_cards = daily_cards_studied.iter().sum::<i32>() as f64 / daily_cards_studied.len() as f64;
        ...
        println!("   Goal achievement rate: {:.1}% ({}/30 days)", goal_achievement_rate * 100.0, goal_achievement_count);
        println!("   Points range: {} - {}", min_daily_points, max_daily_points);
        
        // Assertions to validate simulation results
        assert!(total_cards_studied >= 30, "Should study at least 1 card per day on average");
        assert!(avg_daily_points >= 5.0, "Should earn reasonable points daily");
        assert!(goal_achievement_count >= 5, "Should achieve goal on multiple days");
        assert!(*max_daily_points >= DAILY_GOAL_POINTS, "Should reach daily goal at least once");
    ...
    fn simulate_daily_session(date: NaiveDate, db_path: &std::path::Path) -> Result<(i32, i32, bool), Box<dyn std::error::Error>> {
    ...
    fn simulate_daily_card_count(date: NaiveDate) -> i32 {
        ...
        (base_cards + weekend_modifier + random_modifier).max(8).min(15)
    }
    ...
    fn record_daily_rating_with_path(card_id: i64, rating: Rating, timestamp: i64, db_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn validate_bandit_learning(db_path: &std::path::Path, start_date: NaiveDate) -> Result<bool, Box<dyn std::error::Error>> {
        let conn = Connection::open(db_path)?;
    ...
    fn test_points_system_comprehensive() {
        ...
        let test_cases = vec![
            ...
            (Rating::Good, 2500, 10, 3, 50, 70, "Good default card with combo"),
            (Rating::Hard, 1400, 0, 0, 5, 15, "Hard difficult card"),
        ];
        
        println!("🧮 Points System Comprehensive Test:");
        for (rating, ease_factor, combo, speed_bonus, min_expected, max_expected, desc) in test_cases {
            let points = calculate_points_awarded(rating, ease_factor, combo, speed_bonus);
            println!("   {}: {} points (expected {}-{})", desc, points, min_expected, max_expected);
            
            assert!(points >= min_expected && points <= max_expected, 
                   "Points {} outside expected range {}-{} for {}", points, min_expected, max_expected, desc);
        }
