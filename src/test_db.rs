...
fn test_database(db_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, card_result) in card_rows.enumerate() {
        match card_result {
            Ok((id, nid, due, ivl, factor, lapses)) => {
                println!("    Card {}: id={}, note_id={}, due={}, interval={}, factor={}, lapses={}", 
                         i + 1, id, nid, due, ivl, factor, lapses);
            }
            Err(e) => {
    ...
    println!("  🎲 Random card query test (should be <= 100): {}", random_cards);
...
fn test_load_more_cards_logic(db_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  📊 Simulating load_more_cards with limit={}, failed_today={}, hard_today={}", 
             limit, failed_today.len(), hard_today.len());
    ...
    let uncovered_cards: Vec<_> = stmt.query_map([limit as i64], |row| {
        Ok(Card {
            id: row.get(0)?, 
            note_id: row.get(1)?,
            due: row.get(2)?, 
    ...
    if cards_still_needed > 0 {
        println!("  🎲 Strategy 3: Testing random cards query (need {} more)...", cards_still_needed);
        ...
        let random_cards: Vec<_> = stmt.query_map([cards_still_needed as i64], |row| {
            Ok(Card {
                id: row.get(0)?, 
                note_id: row.get(1)?,
                due: row.get(2)?, 
        ...
        println!("  ✅ Total cards would be loaded: {}", uncovered_cards.len() + random_cards.len());
    } else {
        println!("  ✅ Total cards would be loaded: {} (no additional cards needed)", uncovered_cards.len());
    }
    ...
    let due_cards: Vec<_> = stmt.query_map([today as i64], |row| {
        Ok(Card {
            id: row.get(0)?, 
            note_id: row.get(1)?,
            due: row.get(2)?, 
