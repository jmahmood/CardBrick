...
use crate::ui::{font::TextLayout, sprite::Sprite, CanvasManager, FontManager};
...
impl<'a> StudyingState<'a> {
    ...
    pub fn new(
        ...
        println!(
            "📊 Starting session with {} points already earned today",
            points_today
        );
...
pub fn draw_studying_scene(
    ...
    if total > 0 {
        ...
        if completed != studying_state.last_completed || total != studying_state.last_total {
            ...
            studying_state.cached_progress_size =
                hint_font_manager.size_of_text(&studying_state.cached_progress_text)?;
        ...
        hint_font_manager.draw_line_top_left(
            &studying_state.cached_progress_text,
            text_x as i32,
            text_y as i32,
            canvas_manager,
        )?;
    ...
    if studying_state.mode == StudyingScreenMode::InProgress {
        ...
        if studying_state.points_this_session != studying_state.last_points_today
            || studying_state.cached_score_text.is_empty()
        {
            ...
            let label = if studying_state.is_math_mode {
                "Problems"
            } else {
                "Session"
            };
            studying_state.cached_score_text = format!(
                "{}: {}/{}",
                label, studying_state.points_this_session, session_goal
            );
            studying_state.cached_score_size =
                hint_font_manager.size_of_text(&studying_state.cached_score_text)?;
        ...
        if crate::config::ENABLE_EXPLORE_MODE {
            if let Ok(adopted) = studying_state.db_manager.adopted_today() {
                ...
                let (bg2_x, bg2_y) = canvas_manager
                    .logical_to_screen(10.0, BAR_HEIGHT + 10.0 + (score_h as f32) + 10.0);
        ...
        StudyingScreenMode::InProgress => {
            ...
            if !studying_state.is_answer_revealed {
                ...
                if let Some(layout) = layout_to_draw {
                    ...
                    if y_pos + layout.total_height > content_area_top as i32
                        && y_pos < content_area_bottom as i32
                    {
                ...
                if let Some(layout) = small_front_layout_to_draw {
                        ...
                        && y_pos < content_area_bottom as i32
                    {
                        small_font_manager.draw_layout(
                            layout,
                            margin as i32,
                            y_pos + small_font_ascent as i32,
                            studying_state.show_ruby_text,
                            canvas_manager,
                        )?;
                    }
            ...
            if studying_state.is_answer_revealed {
                if let Some(hint_layout) = &studying_state.hint_layout {
                    ...
                    hint_font_manager.draw_layout(
                        hint_layout,
                        margin as i32,
                        hint_y as i32 + hint_ascent as i32,
                        studying_state.show_ruby_text,
                        canvas_manager,
                    )?;
                }
        ...
        StudyingScreenMode::GoalSplash => {
            ...
            if let Some(layout) = &studying_state.banner_layout {
                ...
                draw_rectangle(
                    ...
                    bg_w,
                    bg_h,
                    Color::from_rgba(0, 0, 0, (128.0 * alpha) as u8),
                );
        ...
        StudyingScreenMode::SessionComplete | StudyingScreenMode::ExhaustedDeck => {
            ...
            if let Some(layout) = &studying_state.banner_layout {
                ...
                if studying_state.mode == StudyingScreenMode::SessionComplete {
                    draw_session_progress_boxes(
                        studying_state,
                        canvas_manager,
                        y_center + layout_height + 10.0,
                    )?;
                }
                ...
                let prompt_text = match studying_state.mode {
                    StudyingScreenMode::SessionComplete => {
                        "Press A to Continue, B for Menu, R for Details"
                    }
...
fn draw_session_progress_boxes(
    studying_state: &mut StudyingState,
    canvas_manager: &CanvasManager,
    y_position: f32,
) -> Result<(), String> {
    ...
    let daily_ratings = if let Some(ref cached) = studying_state.cached_daily_ratings {
        ...
        if studying_state.animation_frame_counter - studying_state.ratings_cache_frame
            < cache_expiry_frames
        {
            cached.clone()
        } else {
...
fn draw_session_details(
    ...
    for layout in &studying_state.detail_layouts {
        if current_y > -50 && current_y < logical_height as i32 + 50 {
            // Only draw visible layouts
            font_manager.draw_layout(
                layout,
                margin,
