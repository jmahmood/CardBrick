...
use super::logic::{
    continue_studying, handle_card_rating, load_card_layouts, load_next_card, update_goal_splash,
    update_rating_toast,
};
use super::{StudyingScreenMode, StudyingState};
...
pub fn handle_studying_input(state: &mut AppState, input: BrickInput) -> Result<(), String> {
    if let GameState::Studying(studying_state) = &mut state.game_state {
        ...
        if studying_state.mode == StudyingScreenMode::InProgress
            && studying_state.current_card.is_none()
        {
            load_next_card(
                studying_state,
                &mut state.font_manager,
        ...
        match input {
                ...
                match studying_state.mode {
                    StudyingScreenMode::InProgress => {
                        if studying_state.is_answer_revealed {
                            ...
                            let total_height = if let (Some(front), Some(back)) = (
                            ...
                            studying_state.scroll_offset =
                                (studying_state.scroll_offset + scroll_speed).min(max_scroll);
                            ...
                            studying_state.card_flip_time =
                                Some(macroquad::time::get_time() as f32);

                            let margin: u32 = 30;
                            let hint_spans = html_parser::parse_html_to_spans(
                                "A:Good B:Again X:Easy Y:Hard [LB:Rewind] [RB:Ruby]",
                            );
                            studying_state.hint_layout =
                                Some(state.hint_font_manager.layout_text_binary(
                                    &hint_spans,
                                    state.config.window_width / 2 - margin * 2,
                    ...
                    StudyingScreenMode::SessionDetails => {
                        ...
                        studying_state.detail_scroll_offset =
                            (studying_state.detail_scroll_offset + 30).min(max_scroll);
                    }
                    _ => {}
                }
            }
                ...
                match studying_state.mode {
                    StudyingScreenMode::InProgress => {
                        if studying_state.is_answer_revealed {
                            ...
                            println!("scroll_offset={}", studying_state.scroll_offset);
                            studying_state.scroll_offset =
                                (studying_state.scroll_offset - scroll_speed).max(0);
                        }
                    }
                    StudyingScreenMode::SessionDetails => {
                        // Scroll up in detail view
                        studying_state.detail_scroll_offset =
                            (studying_state.detail_scroll_offset - 30).max(0);
                    }
                    _ => {}
                }
            }
            ...
            BrickInput::ButtonDown(BrickButton::X) => {
                if studying_state.mode == StudyingScreenMode::InProgress
                    && studying_state.is_answer_revealed
                {
                    rate_card_and_continue(
                        studying_state,
                        Rating::Easy,
...
fn rate_card_and_continue(
    ...
    match rating {
        crate::scheduler::Rating::Again => {
            if let Err(e) = studying_state
                .db_manager
                .record_difficult_card(card_id, "failed")
            {
        ...
        crate::scheduler::Rating::Hard => {
            if let Err(e) = studying_state
                .db_manager
                .record_difficult_card(card_id, "hard")
            {
                eprintln!("Warning: Failed to record difficult card: {}", e);
            }
        }
    ...
    if let Some(ref card_after_rating) = studying_state.current_card {
        studying_state
            .replay_logger
            .log_action(card_after_rating, rating)
...
pub fn build_session_detail_layouts(
    studying_state: &mut StudyingState,
    small_font_manager: &mut crate::ui::FontManager,
) -> Result<(), String> {
    use crate::deck::html_parser;
    ...
    let (failed_cards, hard_cards) = studying_state
        .db_manager
        .get_todays_difficult_cards()
    ...
    let header_text = format!(
        "Today's Challenging Cards ({} cards)",
        challenging_cards.len()
    );
    ...
    for (card_id, rating) in challenging_cards {
        ...
        match studying_state.scheduler.get_card_note_from_db(card_id) {
            Ok(Some((note_id, note))) => {
                println!(
                    "DEBUG: Successfully got note_id: {} and note for card_id: {}",
                    note_id, card_id
                );
...
pub fn calculate_detail_max_scroll(studying_state: &StudyingState) -> i32 {
    ...
    let total_height: i32 = studying_state
        .detail_layouts
        .iter()
...
mod tests {
    ...
    fn test_button_input_conditions() {
        ...
        let is_answer_revealed = true;
        let should_not_rate_different_mode =
            mode == StudyingScreenMode::InProgress && is_answer_revealed;
    ...
    fn test_detail_scroll_max_calculation() {
        ...
        let total_height: i32 = layouts.iter().map(|layout| layout.total_height + 20).sum();
