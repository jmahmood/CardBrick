...
use crate::ui::{FontManager, CanvasManager, font::TextLayout};
use crate::ui::progress::{DailyProgress, load_recent_progress};
    ...
    pub fn new() -> Self {
        ...
        match load_recent_progress(14) {
            Ok(data) => {
                ...
                println!("📊 Loaded {} days of progress data", state.progress_data.len());
            }
            Err(e) => {
...
fn build_progress_layouts(
    ...
    progress_state.header_layout = font_manager.layout_text_binary(&header_spans, 450, false).ok();
    ...
    for day in &progress_state.progress_data {
        ...
        let goal_achieved = day.reward_bin.map_or("?", |achieved| if achieved { "🎯" } else { "○" });
        
        let row_text = format!("{} | {:>5} | {:>6} | {}", 
