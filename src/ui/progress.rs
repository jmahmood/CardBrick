...
pub fn load_recent_progress(
    days_back: usize,
) -> Result<Vec<DailyProgress>, Box<dyn std::error::Error>> {
...
pub fn format_progress_table(progress: &[DailyProgress]) -> String {
    ...
    for day in progress {
        ...
        let success = day
            .reward_bin
            .map_or("?".to_string(), |x| if x { "" } else { "" }.to_string());
...
mod tests {
    ...
    fn test_format_progress_table() {
        let progress = vec![DailyProgress {
