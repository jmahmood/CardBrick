// CardBrick - main.rs
// Phase 3 & 4: Implemented SM-2 Logic with Loading Screen and Progress Bar
...
use ui::{CanvasManager, FontManager, font::TextLayout};

/// A message sent from the loading thread to the main thread.
...
struct AppState<'a> {
    ...
    small_font_manager: FontManager<'a, 'a>, // Font for the shrunk question
    hint_font_manager: FontManager<'a, 'a>,
    current_card: Option<Card>,
    is_answer_revealed: bool,
    // Store pre-calculated layouts for the current card.
    front_layout: Option<TextLayout>,
    back_layout: Option<TextLayout>,
    small_front_layout: Option<TextLayout>, // Layout for the shrunk question
    scroll_offset: i32,
...
pub fn main() -> Result<(), String> {
    ...
    thread::spawn(move || {
        deck::loader::load_apkg(&deck_path, tx);
    });
...
/// Helper function to load the next card and calculate its text layouts.
    ...
    if let Some(scheduler) = state.scheduler.as_mut() {
        ...
        if let Some(card) = &state.current_card {
            if let Some(note) = scheduler.get_note(card.note_id) {
                ...
                // Pre-calculate all layouts for the new card
...
fn run(state: &mut AppState, event_pump: &mut sdl2::EventPump, rx: Receiver<LoaderMessage>) -> Result<(), String> {
    'running: loop {
        if let GameState::Loading = state.game_state {
            match rx.try_recv() {
                Ok(LoaderMessage::Complete(Ok(deck))) => {
                    state.scheduler = Some(Box::new(Sm2Scheduler::new(deck)));
                    state.game_state = GameState::Reviewing;
                    load_next_card(state);
                }
                Ok(LoaderMessage::Complete(Err(e))) => { state.game_state = GameState::Error(e); }
                Ok(LoaderMessage::Progress(p)) => { state.loading_progress = p; }
                Err(TryRecvError::Empty) => {}
                Err(TryRecvError::Disconnected) => { state.game_state = GameState::Error("Loading thread disconnected.".to_string());}
            }
        }
            ...
            if state.is_answer_revealed {
                ...
                if let Some(r) = rating {
                    ...
                    let total_height = if let (Some(front), Some(back)) = (&state.small_front_layout, &state.back_layout) {
                        front.total_height + back.total_height + 20 // 20px padding
                    } else { 0 };
    ...
    state.canvas_manager.with_canvas(|canvas| {
        match &state.game_state {
            ...
            GameState::Reviewing | GameState::Done => {
                ...
                if !state.is_answer_revealed {
                    // Draw only the front layout if answer is not revealed
                    if let Some(layout) = &state.front_layout {
                        state.font_manager.draw_layout(canvas, layout, margin as i32, 40)?;
                    }
                } else {
                    // Draw the shrunk front layout and the back layout
                    ...
                    if let Some(layout) = &state.small_front_layout {
                        ...
                        y_pos += layout.total_height + 20; // Add padding
                    }
                ...
                if state.is_answer_revealed {
                     let hint_layout = state.hint_font_manager.layout_text("A:Good B:Again X:Easy Y:Hard (Up/Down)", 512 - margin * 2)?;
