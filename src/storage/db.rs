...
impl DatabaseManager {
    ...
    #[allow(dead_code)]
    pub fn get_difficult_cards_for_date(
        &self,
        date: &str,
        difficulty_type: Option<&str>,
    ) -> Result<Vec<i64>> {
    ...
    pub fn update_daily_streak(&self) -> Result<i32> {
        ...
        let (current_streak, last_date) = self
            .conn
...
mod tests {
    ...
    fn test_study_event_points_calculation() {
        ...
        manager
            .record_study_event(1, timestamp, 10, 2, 5, 3, 20)
            .unwrap();
        manager
            .record_study_event(2, timestamp + 1, 15, 3, 0, 2, 20)
            .unwrap();
        manager
            .record_study_event(3, timestamp + 2, 0, 1, 10, 0, 11)
            .unwrap();
    ...
    fn test_difficult_cards_date_isolation() {
        ...
        let failed_today = manager
            .get_difficult_cards_for_date(&today, Some("failed"))
            .unwrap();
        let hard_today = manager
            .get_difficult_cards_for_date(&today, Some("hard"))
            .unwrap();
        ...
        let yesterday = (Utc::now().date_naive() - Duration::days(1))
            .format("%Y-%m-%d")
            .to_string();
        let failed_yesterday = manager
            .get_difficult_cards_for_date(&yesterday, Some("failed"))
            .unwrap();
