...
use super::{scanner, CachedDeck, Card, Deck};
...
pub fn load_apkg(path: &Path, tx: Sender<LoaderMessage>) {
    ...
    let result = (|| -> Result<Deck, Box<dyn std::error::Error>> {
        ...
        println!(
            "⏱️  [{}ms] Card count query completed: {} cards",
            start_time.elapsed().as_millis(),
            total_cards
        );
        ...
        println!(
            "⏱️  [{}ms] Loaded {} scheduled cards (due today or overdue)",
            start_time.elapsed().as_millis(),
            scheduled_cards.len()
        );
        ...
        if scheduled_cards.len() < min_cards {
            ...
            println!(
                "Need {} more cards to reach minimum of {}",
                needed_cards, min_cards
            );

            // Get additional cards that aren't already selected
            let (query, params): (String, Vec<rusqlite::types::Value>) =
                if scheduled_cards.is_empty() {
                    ...
                    let mut params: Vec<rusqlite::types::Value> =
                        selected_ids.iter().map(|&id| id.into()).collect();
            ...
            println!(
                "⏱️  [{}ms] Added {} additional cards (random selection) for total of {}",
                start_time.elapsed().as_millis(),
                additional_count,
                scheduled_cards.len()
            );
        ...
        println!(
            "⏱️  [{}ms] Card loading phase complete",
            start_time.elapsed().as_millis()
        );
        ...
        println!(
            "⏱️  [{}ms] TOTAL LOADING TIME - Deck ready",
            start_time.elapsed().as_millis()
        );
        Ok(deck)
    ...
    tx.send(LoaderMessage::Complete(result.map_err(|e| e.to_string())))
        .unwrap();
}
...
mod tests {
    ...
    fn test_load_apkg_minimum_card_selection() {
        ...
        assert!(
            deck.cards.len() >= 10,
            "Should have at least 10 cards, got {}",
            deck.cards.len()
        );
    }
