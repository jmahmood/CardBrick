...
impl MdnsDiscovery {
    ...
    pub async fn discover_services(&self) -> Result<Vec<DiscoveredService>> {
        ...
        if let Ok(mdns_services) = self.discover_via_mdns().await {
            ...
            let discovered: Vec<DiscoveredService> = mdns_services
                .into_iter()
        ...
        if services.is_empty() {
            ...
            if let Ok(udp_services) = self.discover_via_udp_broadcast().await {
                ...
                let discovered: Vec<DiscoveredService> = udp_services
                    .into_iter()
                    .map(DiscoveredService::from)
                    .collect();
                self.add_unique_services(discovered, &mut services, &mut seen_addresses);
    ...
    async fn discover_via_mdns(&self) -> Result<Vec<ServiceInfo>> {
        ...
        let stream = mdns::discover::all(CARDBRICK_SERVICE_TYPE, self.mdns_timeout)?.listen();
        pin_mut!(stream);
    ...
    async fn parse_mdns_response(response: mdns::Response) -> Option<ServiceInfo> {
                ...
                RecordKind::TXT(txt_data) => {
                    ...
                    for txt_entry in txt_data {
                        ...
                        if txt_entry.len() > 255 {
                            ...
                            warn!(
                                "Oversized TXT record ignored: {} bytes, content: {:?}",
                                txt_entry.len(),
                                preview
                            );
                            continue;
        ...
        if address.is_none() && srv_target.is_some() {
            if let Some(target) = srv_target {
                debug!(
                    "No A/AAAA record found, attempting DNS lookup for SRV target: {}",
                    target
                );
        ...
        match (address, port, hostname.clone()) {
            ...
            _ => {
                warn!(
                    "Incomplete mDNS response: addr={:?}, port={:?}, host={:?}",
    ...
    async fn discover_via_udp_broadcast(&self) -> Result<Vec<ServiceInfo>> {
        ...
        let socket =
            UdpSocket::from_std(socket2.into()).context("Failed to convert to tokio UDP socket")?;
    ...
    fn parse_udp_response(&self, response: &str, sender_ip: IpAddr) -> Result<ServiceInfo> {
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(response).context("Failed to parse UDP response JSON")?;
...
impl From<ServiceInfo> for DiscoveredService {
    fn from(info: ServiceInfo) -> Self {
        let name = format!(
            "CardBrick Sync ({} via {})",
            info.hostname, info.discovered_via
        );
...
mod tests {
    ...
    fn test_udp_response_parsing() {
        let discovery = MdnsDiscovery::new();
        let response =
            r#"{"service":"cardbrick","version":"1","port":6429,"hostname":"test-desktop"}"#;
        ...
        assert_eq!(
            result.txt_records.get("proto"),
            Some(&"udp-broadcast".to_string())
        );
    }
    ...
    fn test_from_service_info() {
        ...
        assert_eq!(
            ...
            "CardBrick Sync (test-host via mDNS)"
        );
        assert_eq!(
        ...
        assert_eq!(
            discovered_service.txt_records.get("version"),
            Some(&"1".to_string())
        );
    }
    ...
    fn test_random_udp_responses() {
        ...
        let discovery = MdnsDiscovery::new();
        let valid_response =
            r#"{"service":"cardbrick","version":"1","port":6429,"hostname":"test"}"#;
